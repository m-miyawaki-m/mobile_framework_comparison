...
「**AAR を Capacitor プラグイン化して PWA で動かしたい**」という構成案が技術的に成立しない理由を、配信形態・実行ランタイム・レイヤ境界の観点から原理的に整理する。具体例として DENSO WAVE SP2(外付け BT 5.0 RFID/バーコードスキャナ)を用いる。
...
| 成果物 | 物理的な所在 | ロードする主体 |
|---|---|---|
| **AAR**(`denso-sp2-sdk.aar`)| APK 内 `classes.dex` に展開(Gradle が AAR を解凍 → クラスを dex 化 → APK に同梱)| Android Runtime のクラスローダ(`DexClassLoader`)|
...
DENSO SP2 SDK / Zebra EMDK 等は **AAR(Java/Kotlin)+ ネイティブ共有ライブラリ(`.so`)** の組合せで、Android API への深い依存を持つ:
...
## 5. ケーススタディ — DENSO WAVE SP2

### 5.1 端末の特性

- **形態**: 外付け BT 5.0 セパレート型(スマートデバイスとペアリング)
- **読取**: UHF 920MHz RFID(約 1,300 タグ/秒、〜8m)+ 2D バーコード(QR/DataMatrix/1次元)
- **接続**: Bluetooth 5.0(Classic + LE)、NFC(ペアリング補助)、USB-C(充電/通信)
- **SDK 提供**: Android(AAR と推定)/ iOS / Windows、ベンダー無償ダウンロード
...
| 通信 | DENSO 独自の BT GATT プロトコルは SDK 内に隠蔽、Web Bluetooth で代替不可 |

「Web Bluetooth で SP2 と直接話せばいいのでは?」という発想は理論上ありうるが、現実には:

- SP2 の GATT サービス UUID・キャラクタリスティック構造・データフレーム形式は公開されていない(SDK 内に隠される)
...
| 案 | 概要 | SP2 機能 | PWA 同居 |
|---|---|---|---|
| **A. Capacitor APK + カスタムプラグイン** | AAR を `android/app/libs/` に置き、`@CapacitorPlugin` で JS API を公開 | ◎ フル機能 | ハイブリッドコードベース内で別ビルド(PWA は SP2 非対応で動く) |
| **B. 純ネイティブ APK**(Kotlin)| AAR を直接 build.gradle で取り込み、Compose UI から呼ぶ | ◎ フル機能 | × PWA とコードベース分離 |
| **C. PWA のみ**(SP2 を端末カメラに置換)| 端末カメラ + html5-qrcode/BarcodeDetector で QR のみ | バーコードのみ(端末カメラ品質依存)、UHF RFID は完全不可 | ◎ PWA 単独 |

本件で SP2 の主機能(UHF RFID)が要件なら **案 A または B が必須**。「PWA で SP2 を使う」は要件として両立しない。
...
PWA 配信時はこの図の点線より下が **存在しない** ため、UI 層の分岐で「PWA は SP2 不可」を明示する設計になる。
...
DENSO SP2 SDK は Android Runtime + Bluetooth サブシステム + Context 等を前提に書かれている。これらを満たさないランタイム(=ブラウザ)では動かない。**「とにかく動かしたい」ではなく「動作前提を満たすランタイムは何か」から逆算する**。
...
| **ベンダー AAR(Zebra/Honeywell/DENSO/Datalogic)** | **×** | **×** | ◎(カスタムPlugin)| ◎ |
| **UHF RFID(SP2 / Zebra RFD 等)** | **×** | **×** | ◎ | ◎ |
...
- [`architecture_hybrid.puml`](./architecture_hybrid.puml) の `4_hybrid_plugin_architecture` がカスタムプラグイン実装の構造、`5_hybrid_ht_scanner` が DataWedge Intent の Plugin 化例。SP2 のような外付け SDK 端末も同じパターン(BroadcastReceiver → BluetoothGatt + SDK 呼び出しに変わるだけ)。
...
| 対象 | URL |
|---|---|
| DENSO WAVE SP2 製品ページ | https://www.denso-wave.com/ja/adcd/product/rfid/sp2.html |

---

## 更新履歴

- **2026-05-11**: 初版作成。DENSO SP2 を例に「AAR + Capacitor プラグイン + PWA 配信」の不成立理由をレイヤ境界の観点から整理。
